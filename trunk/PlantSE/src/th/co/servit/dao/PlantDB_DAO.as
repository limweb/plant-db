package th.co.servit.dao
{
	...
	*/
	...
	public class PlantDB_DAO
	{
	
		private var stmt:SQLStatement;		
		private static var instance:PlantDB_DAO;
		public static function getInstance():PlantDB_DAO {
			if( instance == null )
				instance = new PlantDB_DAO( new SingletonLock );
			return instance;
		}
		
	
		public function PlantDB_DAO( lock: SingletonLock) {
		}
		...
		public function getTableContent( resultHandler:Function, faultHandler:Function = null ):void {
			var stmt:SQLStatement = new SQLStatement();
			stmt.sqlConnection = sqlConnection;
			stmt.text = 'SELECT commonname,othername,scientificname,genus,family,treesize,minheight,maxheight,minhedgesize,maxhedgesize,closeplantingdist,longplantingdist,unit,planttype,treeshape,texture,trunk,leaf,bloomingseason,isfruit,fruittype,isherb,flowercolor,isscented,waterrequirement,sunlightrequirement,soiltype,growthrate,deciduousorevergreen,isscreen,makenoise,safeforkids,maintaindifficult,maintaindetail,advantage,reproductionmethod,plantingstyle,landscapearchuse,recommendation,remark,CAST(pic1 as ByteArray) as pic1,CAST(pic2 as ByteArray) as pic2,CAST(pic3 as ByteArray) as pic3 FROM PlantDB';
...
//			stmt.addEventListener(SQLEvent.RESULT,resultHan);
			stmt.addEventListener( SQLErrorEvent.ERROR, faultHandler == null ? sqlErrorHandler : faultHandler );
			stmt.execute();
		}
		
		public function resultHan(e:SQLEvent):void {
